...
import com.dicoding.asclepius.historydata.DatabaseHistory
import com.dicoding.asclepius.historydata.PredictionHistory
...
class HistoryActivity : AppCompatActivity(), PredictionHistoryAdapter.OnDeleteClickListener {
    ...
    private fun loadPredictionHistoryFromDatabase() {
        GlobalScope.launch(Dispatchers.Main) {
            val predictions = DatabaseHistory.getDatabase(this@HistoryActivity).predictionHistoryDao().getAllPredictions()
    ...
    override fun onDeleteClick(position: Int) {
        ...
        if (prediction.result.isNotEmpty()) {
            GlobalScope.launch(Dispatchers.IO) {
                DatabaseHistory.getDatabase(this@HistoryActivity).predictionHistoryDao().deletePrediction(prediction)
            }
